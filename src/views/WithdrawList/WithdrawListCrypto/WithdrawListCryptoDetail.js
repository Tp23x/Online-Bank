...
import { Box, Typography, TableContainer, Table, TableHead, TableRow, TableCell, Divider, Button, TableBody, FormControl, Select, MenuItem, Avatar } from '@mui/material';
...
export default function WithdrawListCryptoDetail() {
    ...
    const [bankAccNoTo, setBankAccNoTo] = useState()
    ...
    //-------------------GET ACCOUNT-----------------
    useEffect(() => {
        api.get(`api/v1.0/withdraw/account-crypto`)
            .then((response) => response.data)
            .then((data) => {
                if (data.status === true) {
    ...
    const Handlesubmit = (e) => {
        api.put(`api/v1.0/withdraw/detail/crypto/${tscba_tscBankAccountID}`,
            { "bankAccNoTo": `${Account}` })
            ...
            .then((data) => {
                if (data.status === true) {
                    ...
                    window.location.href = '/WithdawList/ListCrypto'
    ...
    useEffect(() => {
        if (isLoading4 === false && isLoading2 === false) {
            setIsLoading(false)
    ...
    //console.log("wwww", isLoading);
    console.log("TEst", dataAccount, detail, isLoading, isLoading4, isLoading2);
                ...
                <Box sx={{ marginTop: 5 }}>
                    <TableContainer >
                        <Table sx={{ minWidth: 800 }} aria-label="simple table">
                            ...
                            <TableBody>
                                <StyledTableCell align="left">{moment(detail.tscba_modifiedDate).format("DD-MM-YYYY HH:mm")}</StyledTableCell>
                                <StyledTableCell align="left">{parseFloat(Math.abs(detail.tscba_amount, 10)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</StyledTableCell>
                                <StyledTableCell align="center">{detail.tscba_bankCodeTo ? detail.tscba_bankCodeTo : '-'}</StyledTableCell>
                                <StyledTableCell align="right">{detail.tscba_bankAccNoTo} </StyledTableCell>
                            </TableBody>
                        </Table>
                    </TableContainer>
                            ...
                            {detail.tscba_status === "P" ?
                                <Box sx={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                                    ...
                                    <Box sx={{ display: 'flex', flexDirection: 'column', width: "100%", alignItems: "left" }}>
                                        ...
                                        <Typography sx={{ fontSize: '25px', fontWeight: '500', color: "#138D75" }}>
                                ...
                                <Box sx={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                                    <Box sx={{ display: 'flex', flexDirection: 'column', width: "100%" }}>
                                        <Typography>ยอดแจ้งถอน</Typography>
                                        <Typography sx={{ fontSize: '25px', fontWeight: '500', color: "#535252" }}>
                                            {parseFloat(Math.abs(detail.tscba_amount, 10)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                                        </Typography>
                                    </Box>
                                    <Box sx={{ width: "100%", alignItems: "left" }}>
                                        <Typography>ยอดเงินคงเหลือ</Typography>
                                        <Typography sx={{ fontSize: '25px', fontWeight: '500', color: "#138D75" }}>
                                            {dataAccount.filter((item) => item.mba_bankAccNo === detail.tscba_bankAccNoTo).map((data) =>
                                                parseFloat(data.mba_balance, 10).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'))}
                                        </Typography>
                                    </Box>
                                    {/*  <Box sx={{ display: 'flex', flexDirection: 'column',width: "100%", alignItems: "left" }}>
                                        <Typography>ยอดเงินคงเหลือ</Typography>
                                            <Typography sx={{ fontSize: '25px', fontWeight: '500', color: "#535252" }}>
                                                {dataAccount.filter((item) => item.mba_bankAccNo === detail.tscba_bankAccNoTo).map((data) =>
                                                    parseInt((data.mba_balance - Math.abs(detail.tscba_amount)), 10).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'))}
                                            </Typography>
                                    </Box> */}
                                </Box>
                            }
                    ...
                    {
                        parseFloat(Math.abs(detail.tscba_amount, 10)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,') >= dataAccount.filter((item) => item.mba_bankAccNo === Account).map((data) => parseInt(data.mba_balance, 10).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'))
                            ? '' :
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', marginTop: "20px" }}>
                        ...
                        <Typography sx={{ fontSize: '25px', fontWeight: '500', color: "#ef2851", marginBottom: "20px" }}>
                            {parseFloat(Math.abs(detail.tscba_amount, 10)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                        </Typography>
                    </Box>
                    <Divider />
                    <Box sx={{ display: 'flex', justifyContent: 'center', width: "100%", }}>
                        <Button onClick={() => navigate('/WithdawList/ListCrypto')}
                            sx={{ marginRight: "20px", backgroundColor: "#eb4e6e", '&:hover': { background: '#ef2851' }, color: "#FFF", padding: "7px 55px", marginTop: "23px" }}>
                            กลับ
                        </Button>
                        {
                            parseFloat(Math.abs(detail.tscba_amount, 10)).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,') <= dataAccount.filter((item) => item.mba_bankAccNo === Account).map((data) => parseInt(data.mba_balance, 10).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'))
                                ...
                                <Button onClick={Handlesubmit}
                                    sx={{ backgroundColor: "#30e76a", '&:hover': { background: '#1fc64c' }, color: "#FFFF", padding: "7px 55px", marginTop: "23px", display: detail.tscba_status === "A" ? 'none' : 'block' }}>
                                    ตกลง
                                </Button>
                        }
                        {/* <Button onClick={Handlesubmit}
